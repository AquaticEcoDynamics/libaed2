...
# define SED_REAL REAL(SEDP)
  ...
  ! ------------------------------------------------------
   TYPE aed2_sed_candi_time_t
      ...
      INTEGER  :: fluxon           !# 5       ! If time<fluxon then flux will be zero, if time>fluxon then flux will be default_vals
      INTEGER  :: fluxoff          !# 15      ! If time>fluxoff then flux will be zero
      INTEGER  :: substep          !# 240     ! Hourly if glm is hourly, i.e. 3600 s; CANDI all units are y
      INTEGER  :: driverDT         !# 3600.   ! s Should be 3600 if hourly
   END TYPE aed2_sed_candi_time_t

  ! ------------------------------------------------------
   TYPE aed2_sed_candi_bc_t
   ...
   END TYPE aed2_sed_candi_bc_t

  ! ------------------------------------------------------
   TYPE aed2_sed_candi_param_t
...
!     SED_REAL :: SolidOutputUnit  !#  mmolLsolids !mM
    ...
    !--XMetal--!   For the VCW or Canavan stoichiometry, this is like "z"
      ...
      SED_REAL :: kMnNO3           !#     0.0 ! Turn it off to match VCW 1.0e+2 !mM^-1 y^-1 From Norlem    !1e+5! uM^-1 y^-1 From Norlem
      ...
      SED_REAL :: kFeNO3           !#      0.0 ! Off to match VCW1.0e+1 !Tuning it to reduce NO3 in the surface layers ! 1.0e+2   ! mM^-1 y^-1 From Norlem !1e+5! uM^-1 y^-1 From Norlem
      ...
      SED_REAL :: kCH4SO4          !#      1.0e+1  ! mM^-1 y^-1 VCW k17 !From Brigolin 2009 ! 1.0 !mM^-1 y^-1 From Norlem  !1.0E+03! uM^-1 y^-1 From Norlem
   ...
   !------------------------------------------------------------------------
     ...
     SED_REAL :: dG0MetH2          !# -31.3    ! #Roden and Jin 2011 |# -16.92*8 #kJ mol H2^-1  Bethke 2011
      ...
      SED_REAL :: YDHyAer          !# 68.41 ! /CellWeight !# Roden and Jin 2011 | #0.515375 / CellWeight*GlucoseWeight # mol cells (mol glucose)^-1 Not sure where this came from
      SED_REAL :: YDHyFer          !# 11.95 ! /CellWeight # Roden and Jin 2011 | 0.2 #% Watson 2003
      SED_REAL :: YDenDHy          !# 67.10 ! /CellWeight # Roden and Jin 2011 | #0.333  / CellWeight*GlucoseWeight # mol cells (mol glucose)^-1 Not sure where this came from
   ...
   !------------------------------------------------------------------------
      ...
      SED_REAL :: kNO3             !#   3.0E-3     ! WVC96 27.0E-03     ! mM from Megonigal         ! 2.5e-2 !mM From Norlem    !25! uM from Norlem
      SED_REAL :: kpNO3            !#   3.0E-3     ! WVC96 27.0E-03     ! mM from Megonigal         ! 2.5e-2 !mM From Norlem    !25! uM from Norlem
      SED_REAL :: kMnO2            !#   16.0E-0    ! WVC96 13.3E-03     ! mM from Brigolin et al. 2009      ! 1.4 !mmol/g From Norlem   !1400! umol/g from Norlem
      SED_REAL :: kpMnO2           !#   16.0E-0    ! WVC96 13.3E-03     ! mM from Brigolin et al. 2009      ! 1.4 !mmol/g From Norlem   !1400! umol/g from Norlem
      SED_REAL :: kFeOH            !#   100.0E-0   ! WVC96 87.0E-03     ! mM from Brigolin et al. 2009      ! 18.0 !mmol/g From Norlem  !18000! umol/g from Norlem
      SED_REAL :: kpFeOH           !#   100.0E-0   ! WVC96 87.0E-03     ! mM from Brigolin et al. 2009      ! 18.0 !mmol/g From Norlem  !18000! umol/g from Norlem
      SED_REAL :: kSO4             !#   1600.0E-3  ! WVC96 1180.0E-03   ! mM from Brigolin et al. 2009      ! 0.8 !mM From Norlem   !800! uM from Norlem
      SED_REAL :: kpSO4            !#   1600.0E-3  ! WVC96 1180.0E-03   ! mM from Brigolin et al. 2009      ! 0.8  !mM From Norlem  !800! uM from Norlem
      ...
      SED_REAL :: lMnO2            !#  16e-0     ! umol g sediment^-1 = umol cm^-3 sediment - need to normalize by density, porosity
      ...
      SED_REAL :: kanh4
      SED_REAL :: kapo4
   END TYPE aed2_sed_candi_param_t

  ! ------------------------------------------------------
   TYPE aed2_candi_stioc_coefs_t
     ...
     REAL                      :: fracNH2
     REAL                      :: fracPH2
   END TYPE aed2_candi_stioc_coefs_t
