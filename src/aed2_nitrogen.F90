...
!BEGIN
   ...
   data%id_nox = aed2_define_variable('nit','mmol/m**3','nitrate',             &
   ...
   IF( simN2O>0 ) THEN
     ...
     IF( simN2O>1 ) THEN ! Advanced model requires NO2
       data%id_no2 = aed2_define_variable('no2','mmol/m**3','nitrate',         &
   ...
   IF (data%use_oxy) data%id_oxy = aed2_locate_variable(nitrif_reactant_variable)
   ...
   IF (data%use_sed_model_n2o .and. simN2O>0 ) &
     data%id_Fsed_n2o = aed2_locate_global_sheet(Fsed_n2o_variable)
   data%use_sed_model_no2 = Fsed_no2_variable .NE. ''
   IF (data%use_sed_model_no2 .and. simN2O>1 ) &
     data%id_Fsed_no2 = aed2_locate_global_sheet(Fsed_no2_variable)
   ...
   data%id_nitrf   = aed2_define_diag_variable('nitrif','mmol/m**3/d','nitrification rate')
   ...
   IF( simN2O>0 ) THEN
    ...
    IF( simN2O>1 ) data%id_sed_no2 = aed2_define_sheet_diag_variable('sed_no2','mmol/m**2/d','no2 sediment flux')
   ENDIF

   IF( simWetDeposition .OR. simDryDeposition ) THEN
    data%id_atm_dep = aed2_define_sheet_diag_variable('atm_din_flux','mmol/m**2/d','din atmospheric deposition flux')
   ENDIF
   ...
   IF( simN2O>0 )         data%id_wind    = aed2_locate_global_sheet('wind_speed')
   IF( simN2O>0 )         data%id_E_depth = aed2_locate_global('layer_ht')
   IF( simN2O>0 )         data%id_cell_vel= aed2_locate_global('cell_vel')! needed for k600
  !IF( simN2O>0 )         data%id_E_tau   = aed2_locate_global('taub')    ! tau to be converted to velocity
  !IF( simN2O>0 )         data%id_E_dens  = aed2_locate_global('density') ! density needed for tau-vel
...
!BEGIN
  ...
  IF( data%simN2O>0 ) THEN
     ...
     kn2o_trans = aed2_gas_piston_velocity(wind_hgt,wind,temp,salt,vel=vel,    &
